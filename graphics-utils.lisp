...
(defun aabb-expand (aabb vertex)
  (let* ((res   (or aabb (list 1e10 1e10 -1e10 -1e10)))
         (max-x (aabb-max-x res))
         (max-y (aabb-max-y res))
         (min-x (aabb-min-x res))
         (min-y (aabb-min-y res)))
    (when (> (elt vertex 0) max-x)
      (setf (elt res 2) (elt vertex 0)))
    (when (> (elt vertex 1) max-y)
      (setf (elt res 3) (elt vertex 1)))
    (when (< (elt vertex 0) min-x)
      (setf (elt res 0) (elt vertex 0)))
    (when (< (elt vertex 1) min-y)
      (setf (elt res 1) (elt vertex 1)))
    res))
